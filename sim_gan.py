...
# training params
#
#TODO: Play around with the params to see optimal values

nb_steps = 40 # 10000 makes kernel time out
batch_size = 49 #49
k_d = 1 #1  # number of discriminator updates per step
k_g = 20  # number of generative network updates per step
log_interval = 8 # initially 100
pre_steps = 5 #20 # for pretraining
...
def refiner_network(input_image_tensor):
    ...
    def resnet_block(input_features, nb_features=64, nb_kernel_rows=3, nb_kernel_cols=3):
        ...
        # y = layers.Concatenate([input_features, y], mode='sum')
...
def discriminator_network(input_image_tensor):
    ...
    """
    #TODO: Change strides to fit 1D image
    x = layers.Convolution2D(96, (3, 3), padding='same', strides=(2, 2), activation='relu')(input_image_tensor)
...
def adversarial_training(synthesis_eyes_dir, mpii_gaze_dir, refiner_model_path=None, discriminator_model_path=None):
    ...
    def self_regularization_loss(y_true, y_pred):
        delta = 0.00001 # 0.0001  # FIXME: need to figure out an appropriate value for this
    ...
    # data generators
    #
    # TODO: figure out the appropriate preprocessing func
    # datagen = image.ImageDataGenerator(samplewise_center=True, samplewise_std_normalization=True,
    #                                    data_format="channels_last")
    datagen = image.ImageDataGenerator(
        preprocessing_function=None, #applications.xception.preprocess_input,
        data_format='channels_last')
    ...
    # print(synthesis_eyes_dir[20])

    synthetic_generator = datagen.flow(
        x=np.reshape(synthesis_eyes_dir, (2400, 1, 1200, 1)), # synthetic
        **flow_params
    )

    real_generator = datagen.flow(
        x=np.reshape(mpii_gaze_dir, (600, 1, 1200, 1)), #real
        **flow_params
    )
    ...
    # plt.plot(synthetic_image_batch[0][0].T[0])
    # plt.show()
    ...
    if not refiner_model_path:
        ...
        for i in range(pre_steps): #TODO: initially 1000
            synthetic_image_batch = get_image_batch(synthetic_generator)
    ...
    if not discriminator_model_path:
        ...
        for _ in range(1): #TODO: initially 100
    ...
    ref_loss_plot = []
    disc_loss_real_plot = []
    disc_loss_refined_plot = []

    # see Algorithm 1 in https://arxiv.org/pdf/1612.07828v1.pdf
    # plot_data = []
    synthetic_image_batch_plotting = get_image_batch(synthetic_generator)
    sim = synthetic_image_batch_plotting[0][0].T[0]
    # plt.plot(sim)
    # plt.clr()
    # plt.show()
    refined_over_steps = []
    ...
    for i in range(nb_steps):
        ...
        ref_loss_plot.append((combined_loss/(k_g * 2))[0])
        ...
        for _ in range(k_d):
            ...
            disc_loss_refined = np.add(discriminator_model.train_on_batch(refined_image_batch, y_refined),
                                       disc_loss_refined)
        disc_loss_real_plot.append(disc_loss_real / (k_g * 2))
        disc_loss_refined_plot.append(disc_loss_refined / (k_g * 2))

        refined_images_plotting = refiner_model.predict_on_batch(synthetic_image_batch_plotting)

        for i in range(len(sim)):
            diff.append(np.abs(sim[i] - refined_images_plotting[0][0].T[0][i]))
        ...
        if not i % log_interval:
            ...
            #TODO: Save raw image data and plot later

            # print(synthetic_image_batch[0][0].T[0])
            # s = synthetic_image_batch[0][0].T[0]
            # r = refined_images[0][0].T[0]
            # plot_data.append((s,r))
            # def get_plots(s, r):
            #     return s, r
            refined_over_steps.append((i, refined_images_plotting[0][0].T[0]))
            ...
            # log loss summary
            print('Refiner model loss: {}.'.format(combined_loss / (k_g * 2)))
            print('Discriminator model loss real: {}.'.format(disc_loss_real / (k_d * 2)))
            print('Discriminator model loss refined: {}.'.format(disc_loss_refined / (k_d * 2)))
        ...
        axs[0].plot(sim)
        axs[0].set_xlabel('Simulated', fontsize=8)
        axs[1].plot(refined_images_plotting[0][0].T[0])
        axs[1].set_xlabel('Refined at step ' + str(i), fontsize=8)
        # plt.title('Step: {} of {}.'.format(i, nb_steps))
        plt.savefig('fig_at_step_' + str(i) + '.png')
        plt.close()
        # plt.show()

        syn = np.array([synthetic_image_batch_plotting[0][0].T[0]])
        ref = np.array([refined_images_plotting[0][0].T[0]])
        comb = np.concatenate((syn, ref), axis=0)

        name = 'data_at_step_' + str(i) + '.csv'
        np.savetxt(name, comb, delimiter=',', fmt='%1.15f')

    model_checkpoint_base_name = os.path.join(autoxrd_path, '{}_model_step_{}.h5')
    refiner_model.save(model_checkpoint_base_name.format('refiner_processed_exp', nb_steps))
    discriminator_model.save(model_checkpoint_base_name.format('discriminator', nb_steps))

    print(len(refined_over_steps))
    refined_images_plotting = refiner_model.predict_on_batch(synthetic_image_batch_plotting)
    refined_over_steps.append((nb_steps, refined_images_plotting[0][0].T[0]))
    fig, axs = plt.subplots(len(refined_over_steps) + 1, sharex=True, sharey=True)
    axs[len(refined_over_steps)].plot(sim)
    axs[len(refined_over_steps)].set_xlabel('Simulated', fontsize=8)
    for n in range(len(refined_over_steps)):
        axs[n].plot(refined_over_steps[n][1])
        axs[n].set_xlabel("Refined at step " + str(refined_over_steps[n][0]) + " out of " + str(nb_steps), fontsize=8)
    ...
    plt.plot(ref_loss_plot, "-b", label="refiner loss")
    plt.plot(disc_loss_real_plot, "-r", label="disc real loss")
    plt.plot(disc_loss_refined_plot, "-g", label="disc refined loss")
    plt.legend()
    plt.show()

def main(synthesis_eyes_dir, mpii_gaze_dir, refiner_model_path, discriminator_model_path):
    return adversarial_training(synthesis_eyes_dir, mpii_gaze_dir, refiner_model_path, discriminator_model_path)
